    ...
    <div id="main">
        <div class="inner">
            ...
            <section>
                ...
                <header class="major">
                    <h2>
                        <a href="<c:url value="/board/${boardBean.brd_key}?page=${param.page != null ? param.page : 1}"/>">${boardBean.brd_name}</a>
                    </h2>
                </header>
                <hr>
                ...
                <div class="row">
                    ...
                    <div class="col-3 col-3-medium">
                        <p class="text-mute"><spring:message
                                code="table.likes"/> ${postBean.post_like - postBean.post_dislike}</p>
                    </div>
                    <div class="col-3 col-3-medium">
                        <p class="text-mute"><spring:message code="table.hit"/> ${postBean.post_hit}</p>
                    </div>
                    <div class="col-3 col-3-medium">
                        <p class="text-mute"><spring:message code="table.dateWrite"/> <fmt:formatDate
                                value="${postBean.post_datetime}" pattern="yy-MM-dd HH:mm"/></p>
                    </div>
                </div>
                ...
                <div class="btn-container">
                    <c:if test="${member.mem_id == postBean.mem_id}">
                        <button onclick="location.href = '<c:url
                                value="/board/${boardBean.brd_key}/update?post_id=${postBean.post_id}"/>'">
                            <spring:message code="button.update"/></button>
                        <button onclick="location.href = '<c:url
                                value="/board/${boardBean.brd_key}/delete?post_id=${postBean.post_id}"/>'">
                            <spring:message code="button.delete"/></button>
                    </c:if>
                    <button class="primary" onclick="location.href = '<c:url
                            value="/board/${boardBean.brd_key}?page=${pageBean.currentPage}"/>'"><spring:message
